...
then a fixed set of viewer photos compared byte for byte with a reference.
    ...
    .venv/Scripts/python tools/diagnostics/run_checks.py
    .venv/Scripts/python tools/diagnostics/run_checks.py --only test_menu,check_zones
    .venv/Scripts/python tools/diagnostics/run_checks.py --no-photos
    .venv/Scripts/python tools/diagnostics/run_checks.py --photos-only
    .venv/Scripts/python tools/diagnostics/run_checks.py --make-baseline
...
DIAGNOSTICS = os.path.join(TOOLS, "diagnostics")
...
PHOTOS = [
    ...
    ("L03D1_area_no_outside", ["L03D1", "--areaboxes", "--no-area-outside"]),
...
def run_checks(names):
    ...
    for name in names:
        start = time.time()
        proc = subprocess.run([sys.executable, os.path.join(DIAGNOSTICS, name + ".py")], cwd=PROJECT,
...
def take_photos(make_baseline):
    ...
    with tempfile.TemporaryDirectory(prefix="bblit_photos_") as tmp:
        for name, args in PHOTOS:
            ...
            proc = subprocess.run([sys.executable, os.path.join(TOOLS, "viewer.py")] + args
...
def main():
    ...
    if args.make_baseline or args.photos_only or not (args.no_photos or (args.only and not args.photos)):
        photos = take_photos(args.make_baseline)
