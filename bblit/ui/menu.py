...
class Number(Item):
    """`stops`: the only values allowed, in order, instead of a fixed step
    (a value out of the list still moves to the nearest one that follows).
    `labels`: value -> text key, for a value that also has a name of its own.
    """

    def __init__(self, item_key, fetch, store, minimum, maximum, increment=1, number_format="{:.0f}",
                 desc=None, stops=None, labels=None):
        super().__init__(item_key, desc)
        self.fetch, self.store = fetch, store
        self.minimum, self.maximum, self.increment, self.number_format = minimum, maximum, increment, number_format
        self.stops, self.labels = stops, labels or {}

    def value_text(self):
        value = self.fetch()
        shown = self.number_format.format(value)
        if value in self.labels:
            shown = f"{shown} {t(self.labels[value])}"
        return f"‹ {shown} ›"
    ...
    def change(self, increment, menu):
        if self.stops:
            current = self.fetch()
            if increment > 0:
                new = next((s for s in self.stops if s > current), self.stops[-1])
            else:
                new = next((s for s in reversed(self.stops) if s < current), self.stops[0])
        else:
            new = max(self.minimum, min(self.maximum, self.fetch() + increment * self.increment))
        self.store(new)
...
class Menu:
    ...
    def _forget_areas(self):
        ...
        the same one was rebuilt."""
        self._areas, self._areas_list = [], None
    ...
    def rebuild(self):
        ...
        A page can come back SHORTER (the level page has one line per group of
        entities, and another level has fewer): the cursor is brought back
        inside the list, onto an item that can be selected. Leaving it out of
        the list made the whole drawing fail with an IndexError, and with it
        the viewer (`test_level_change.py`).
        """
        for item in self.stack:
            item[1] = self.pages[item[0]].build_items()
            item[2] = self._cursor_inside(item[1], item[2])
        self._forget_areas()
        self.dirty = True

    @staticmethod
    def _cursor_inside(menu_items, cursor):
        """`cursor` brought back inside `menu_items`, on a selectable item:
        the nearest one at or above it, else the first one, else 0."""
        if not menu_items:
            return 0
        cursor = max(0, min(cursor, len(menu_items) - 1))
        if menu_items[cursor].selectable:
            return cursor
        above = [i for i in range(cursor, -1, -1) if menu_items[i].selectable]
        if above:
            return above[0]
        below = next((i for i, v in enumerate(menu_items) if v.selectable), 0)
        return below
    ...
    def _item_at(self, y):
        """The item under `y`, or None. The areas are used only while they
        belong to the list now on top: after a rebuild they are indexes into
        a list that is no longer there."""
        if not self.stack or self._areas_list is not self.stack[-1][1]:
            return None
