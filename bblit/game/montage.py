...
def start_step(obj: dict | None) -> dict | None:
    """The step the game starts the object in.
...
def marker_animation(sec4: bytes, resources: list[int], res: dict, obj: dict | None = None,
                     role: int | None = None):
    """The transforms of every part, frame by frame, or None without a rig.

    Same choice of rig and pose as `animation`; used to hang a clone on an
    attachment marker and have it follow that bone (findings 279, 317).
    """
    r, pose = choose_sources(sec4, resources, res, obj, role=role)
    if r is None or pose is None:
        return None
    frames = rigmod.animation_per_part(sec4, r["offset"], r["size"], pose["offset"], pose["size"])
    return frames or None
